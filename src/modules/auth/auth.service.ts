...
export class AuthService {
  ...
  async login(
    email: string,
    password: string,
    merge?: { sessionId: string; storeId: string },
  ) {
    ...
    if (merge?.sessionId && merge?.storeId) {
      await this.cartService.mergeOnLogin(merge.sessionId, user.id, merge.storeId);
    }
  ...
  private async issueTokens(userId: string, role: 'USER' | 'ADMIN') {
    ...
    const access = await this.jwt.signAsync(
      { sub: userId, role },
      { secret: process.env.JWT_SECRET!, expiresIn: process.env.JWT_EXPIRES || '15m' },
    );

    const refresh = await this.jwt.signAsync(
      { sub: userId, jti },
      { secret: process.env.JWT_SECRET!, expiresIn: `${process.env.REFRESH_EXPIRES_DAYS || 30}d` },
    );
